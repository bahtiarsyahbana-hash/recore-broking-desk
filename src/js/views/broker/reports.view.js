 ...
 */
const exposurePager = createPager(() => reportsView.refresh(), "exposure");
const cedantPager = createPager(() => reportsView.refresh(), "cedant-loss");
...
function exposureTable() {
  ...
  const page = paginate(rows, exposurePager.page);
  ...
  </table></div>` + paginationControls(page, { unit: "markets", name: exposurePager.name });
}
...
const cedantLossTable = () => {
  ...
  const page = paginate(rows, cedantPager.page);
  ...
  </table></div>` + paginationControls(page, { unit: "cedants", name: cedantPager.name });
};
...
export const reportsView = {
  ...
  mount() {
    // Two independent lists on this view, so each keeps its own page.
    exposurePager.wire("#view-root");
    cedantPager.wire("#view-root");
  },
